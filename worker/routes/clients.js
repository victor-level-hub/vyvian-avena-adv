...
async function createClient(request, env) {
  ...
  const { id, name, email, phone, country, identification, practice_area, notes, honorarios_total, honorarios_parcelas, contract_start_date, address, nationality, marital_status, rg, birth_date, birth_place, doc_type, doc_number, doc_validity, niss, filiation, person_type, rep_name, rep_role, duns, process_summary, emails, phones } = body || {};
  if (!id || !name || !country) {
  ...
  try {
    await env.DB.prepare(`
      INSERT INTO clients (id, name, email, phone, country, identification, practice_area, status, notes, honorarios_total, honorarios_parcelas, contract_start_date, address, nationality, marital_status, rg, birth_date, birth_place, doc_type, doc_number, doc_validity, niss, filiation, person_type, rep_name, rep_role, duns, process_summary, emails, phones)
      VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
...
async function updateClient(request, env, clientId) {
  ...
  for (const key of allowed) {
    if (body[key] !== undefined) {
      updates.push(`${key} = ?`);
      params.push((key === 'emails' || key === 'phones') ? serializeContacts(body[key]) : body[key]);
    }
  }
