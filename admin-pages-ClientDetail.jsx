...
export default function ClientDetail() {
  ...
  const handleOpenReceipt = async (installmentId) => {
    setGeneratingReceipt(installmentId);
    try {
      // Faz fetch autenticado e abre o blob como URL temporário
      ...
      // Revogar depois de 1 minuto para libertar memória
      setTimeout(() => URL.revokeObjectURL(url), 60000);
    } catch (err) {
      alert('Erro a gerar recibo: ' + err.message);
    } finally {
                    ...
                    <td>
                      {i.status === 'paid' ? (
                        <a
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            handleOpenReceipt(i.id);
                          }}
                        >
                          {generatingReceipt === i.id ? 'A gerar…' : 'Recibo'}
                        </a>
                      ) : (
                        <a
                          href="#"
